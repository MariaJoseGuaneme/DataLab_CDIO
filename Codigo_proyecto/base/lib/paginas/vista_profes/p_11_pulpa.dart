...
class _PagInicio11_pulpaState extends State<PagInicio11_pulpa> {
  ...
  final TextEditingController _pacidoascorbicoController =
      TextEditingController();
  final TextEditingController _pacidocitricoController =
      TextEditingController();
  final TextEditingController _pbenzoatosodioController =
      TextEditingController();
  final TextEditingController _psorbatopotasioController =
      TextEditingController();
  final DatabaseHelper _databaseH =
  ...
  void _cargarDatos() async {
    final double pPulpa =
        await _databaseH.getNumericValue(practica, 'p_pulpa', idGrupo);
    final double pacidoascorbico = await _databaseH.getNumericValue(
        practica, 'p_acido_ascorbico', idGrupo);
    final double pacidocitrico =
        await _databaseH.getNumericValue(practica, 'p_acido_citrico', idGrupo);
    final double pbenzoatosodio = await _databaseH.getNumericValue(
        practica, 'p_benzonato_sodio', idGrupo);
    final double psorbatopotasio = await _databaseH.getNumericValue(
        practica, 'p_sorbato_potasio', idGrupo);
    setState(() {
      _ppulpaController.text = pPulpa == 0.0 ? "" : pPulpa.toString();
      _pacidoascorbicoController.text =
          pacidoascorbico == 0.0 ? "" : pacidoascorbico.toString();
      _pacidocitricoController.text =
          pacidocitrico == 0.0 ? "" : pacidocitrico.toString();
      _pbenzoatosodioController.text =
          pbenzoatosodio == 0.0 ? "" : pbenzoatosodio.toString();
      _psorbatopotasioController.text =
          psorbatopotasio == 0.0 ? "" : psorbatopotasio.toString();
    });
  }
  ...
  void _guardarP_pulpa() async {
    ...
    await _dbManager.insertSingleDataPractica(
        practica, 'p_pulpa', ppulpa, idGrupo, context);
  }
  ...
  void _guardarP_ascorbico() async {
    ...
    await _dbManager.insertSingleDataPractica(
        practica, 'p_acido_ascorbico', pascorbico, idGrupo, context);
  }
  ...
  void _guardarP_citrico() async {
    ...
    await _dbManager.insertSingleDataPractica(
        practica, 'p_acido_citrico', pcitrico, idGrupo, context);
  }
  ...
  void _guardarP_benzonato() async {
    ...
    await _dbManager.insertSingleDataPractica(
        practica, 'p_benzonato_sodio', pbenzonato, idGrupo, context);
  }
  ...
  void _guardarP_sorbato() async {
    ...
    await _dbManager.insertSingleDataPractica(
        practica, 'p_sorbato_potasio', psorbato, idGrupo, context);
  }
                  ...
                  children: [
                    ...
                    TextField(
                      ...
                      decoration: const InputDecoration(
                        border: OutlineInputBorder(),
                        labelText:
                            'Ingrese el procentaje de sorbato de potasio',
                      ),
                      keyboardType:
                          const TextInputType.numberWithOptions(decimal: true),
                      inputFormatters: [
